(function () {
  ...
  // unified search overlay — works on both the store and the corporate site
  // products are fetched from WooCommerce; pages are hardcoded below
  ...
  // load products from the store; any searches before this resolves still work with pages only
  fetch('http://diploma.local/wp-json/branded-search/v1/products')
    ...
    .catch(function () { /* store unreachable — search still works with pages */ });
  ...
  // bind to any element with aria-label="Search" in the header
